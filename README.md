...
**Made to stick.** The plan behind all this is in [docs/LEARNING_DESIGN.md](docs/LEARNING_DESIGN.md); what's recommended but not built yet is in [docs/RECOMMENDATIONS.md](docs/RECOMMENDATIONS.md). In short:
...
- **Smart App Control (or a school's app policy) is fine: keep it on.** It blocks a few rare, unsigned files the app uses; the app works around each one. The voice speaks from prepared pronunciations (the voice itself runs on Microsoft's signed engine), pictures come from ready-made PNGs, and the speech check uses Windows' own German speech recognition when the German speech pack is installed. If the voice still can't start, the app uses the computer's own German voice instead. If there's none yet: *Settings → Time & language → Speech → Add voices → Deutsch*, then restart the app. (On a Mac: *System Settings → Accessibility → Spoken Content*; on Linux: `sudo apt install espeak-ng`.) The same policy may block the speech checker; speaking turns then just aren't checked. To get everything back on your own computer, Smart App Control can be switched off in *Windows Security → App & browser control* (note: Windows can't switch it on again without a reset); on a school computer, ask IT to allow the app's folder.
...
- After adding or changing German text or pictures, also run `python tools/phoneme_cache.py` (prepared pronunciations) and `python tools/render_pictures.py` (ready-made pictures); the tests remind you.
